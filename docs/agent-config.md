...
| `experimental` | `{ codeMode?: false \| "eager" \| "lazy"; workflow?: { world?: string } }`     | unset            | Opt-in settings that can change or disappear in any release. Treat them as unstable. `codeMode` runs the agent's ordinary executable tools through a `code_mode` sandbox tool instead of direct tool calls: the model writes JavaScript that calls them as `tools.name(input)`, and the sandboxed tools leave the direct model surface. `"eager"` inlines every sandboxed tool signature in the tool description; `"lazy"` lists names only and generated code discovers signatures at runtime via `search_tools`/`describe_tools`. `workflow.world` selects the Workflow world package backing session state, queues, hooks, and streams on the root agent. |
